...
class GeminiVertexClient:
    ...
    def __make_payload(self, user_prompt: str) -> List[types.Content]:
        ...
        # User prompts
        user_content = types.Content(role="user", parts=[types.Part.from_text(text=user_prompt)])
        ...
        try:
            if using_multi_turn:
                ...
                if not self.chat:
                    self.chat = self.client.chats.create(
                        ...
                        config=types.GenerateContentConfig(
                            system_instruction=system_prompt if system_prompt else None, **kwargs
                        ),
                    )
                ...
                response = self.client.models.generate_content(
                    model=self.model,
                    contents=payloads,
                    config=types.GenerateContentConfig(system_instruction=system_prompt, **kwargs),
                )
